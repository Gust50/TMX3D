...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CGFloat h=[TMXReplyCommentCell heightForTableView:_tableView config:^(UITableViewCell *cell) {
...
-(void)setSubReplyModel:(TMXHomeModelDetailCommentSubListModel *)subReplyModel{
    ...
    [_icon sd_setImageWithURL:[NSURL URLWithString:subReplyModel.avatar] placeholderImage:nil];
    _userNameLab.text=subReplyModel.fromUserName;
    NSDate *date = [[KBDateFormatter shareInstance]dateFromTimeInterval:([subReplyModel.createdDate doubleValue]/1000.0)];
    _timeLab.text=[NSDate caculatorTime:date];
    _contentLab.text=subReplyModel.comment;
