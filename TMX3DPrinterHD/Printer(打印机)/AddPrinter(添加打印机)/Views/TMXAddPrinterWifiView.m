...
-(void)updateConstraints{
    ...
    [_loginView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(self.width/1.5, 120));
    }];
...
- (void)selectWifiName:(UIButton *)sender
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickIsSelectWifi:centerPoint:width:)]) {
    
        [self.delegate clickIsSelectWifi:sender.selected centerPoint:self.center width:_loginView.width];
