//
//  TMXBindPrinterTaskListTableViewCell.m
//  TMX3DPrinterHD
//
...
@interface TMXBindPrinterTaskListTableViewCell ()
@property (nonatomic, strong) UIView *bottomLine;

@end

@implementation TMXBindPrinterTaskListTableViewCell
...
- (KBtopImgBottomTextBtn *)startPrintBtn{

    if (!_startPrintBtn) {
        _startPrintBtn = [[KBtopImgBottomTextBtn alloc] init];
        _startPrintBtn.nameContent = @"开始打印";
        _startPrintBtn.textColor = RGBColor(53, 53, 53);
        _startPrintBtn.textFont = [UIFont systemFontOfSize:12];
        _startPrintBtn.iconUrl = @"StartPrint";
    }
    return _startPrintBtn;
}
...
- (void)initUI{
    ...
    [self addSubview:self.startPrintBtn];
...
- (void)updateConstraints{
    ...
    [_cancelTaskBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(_taskSaveBtn.mas_left).with.offset(-20);
        make.centerY.equalTo(weakSelf.mas_centerY);
        make.height.equalTo(@55);
        make.width.equalTo(@55);
    }];
    
    [_startPrintBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(_cancelTaskBtn.mas_left).with.offset(-20);
        make.centerY.equalTo(weakSelf.mas_centerY);
        make.height.equalTo(@55);
        make.width.equalTo(@55);
    }];
