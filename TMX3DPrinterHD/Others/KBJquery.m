...
@property (nonatomic, assign) CGFloat pageControlW;
...
//            LRLog(@"----------------当前大小------------->%@",NSStringFromCGSize(self.frame.size));
...
//    _pageControl.center=CGPointMake(self.center.x, self.frame.size.height-20);
    self.pageControlW=pageSize.width;
    self.pageControlH=pageSize.height;
    [self updateConstraints];
...
-(void)updateConstraints{
    ...
    [_pageControl mas_updateConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(@(_pageControlW));
        make.height.mas_equalTo(@(_pageControlH));
    }];
}

-(void)updateUI:(CGRect)frame{
